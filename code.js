...
const HUBSPOT_SERVERLESS_ENDPOINT = 'https://www.activesgcircle.gov.sg/_hcms/api/update_data';
const HUBDB_ROW_ID_COLUMN = 1; // Column A for storing HubDB row ID (hs_id)
const SYNC_STATUS_ID_COLUMN = 11; // Column A for storing HubDB row ID (hs_id)
const SYNC_MESSAGE_ID_COLUMN = 12; // Column A for storing HubDB row ID (hs_id)  
...
function detectChanges(currentData, storedData) {
  ...
  Object.keys(currentData).forEach(uniqueId => {
    ...
    if (!stored) {
      ...
      const changedFields = [];
      const fieldsToCheck = ['match_no', 'date_and_time', 'category', 'round', 'pitch', 'team_1', 'team_1_score', 'team_2_score', 'team_2'];
...
function handleBatchDeleteSuccess(deleteChanges, sheet, responseText) {
  ...
  if (batchResponseData.success) {
    ...
    const errorMessage = `Batch deletion failed: ${batchResponseData.message || 'Unknown error'}`;
    deleteChanges.forEach(change => {
